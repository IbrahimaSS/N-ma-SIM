      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <Card className="p-4">
          ...
          <div className="flex items-start gap-4 mb-6">
            <div className="p-3 bg-gray-50 rounded-lg border border-border-light"><RefreshCcw className="w-6 h-6 text-primary" /></div>
            <div><p className="text-sm text-text-muted">{t.service}</p><p className="font-bold text-primary">{t.serviceVal}</p></div>
          </div>
          <div className="flex items-start gap-4 mb-8">
            <div className="p-3 bg-gray-50 rounded-lg border border-border-light"><Receipt className="w-6 h-6 text-primary" /></div>
            <div><p className="text-sm text-text-muted">{t.amount}</p><p className="font-bold text-primary text-xl">10 000 GNF</p></div>
          </div>
          <div className="bg-accent/10 border border-accent/30 rounded-xl p-4 flex justify-between items-center mb-6">
            ...
            <div className="font-bold text-primary text-lg">10 000 GNF</div>
          </div>
        ...
        <Card className="p-4">
          ...
          <div className="flex flex-col gap-4">
            <div className={`p-4 rounded-xl border-2 flex items-center cursor-pointer transition-colors ${method === 'orange-money' ? 'border-accent bg-accent/5' : 'border-border-light hover:border-primary/20'}`} onClick={() => { setMethod('orange-money'); setOmStep('initial'); }}>
              <div className="flex items-center gap-4">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${method === 'orange-money' ? 'border-accent' : 'border-gray-300'}`}>{method === 'orange-money' && <div className="w-2.5 h-2.5 bg-accent rounded-full"></div>}</div>
            ...
            <div className={`p-4 rounded-xl border-2 flex items-center cursor-pointer transition-colors ${method === 'visa' ? 'border-primary bg-primary/5' : 'border-border-light hover:border-primary/20'}`} onClick={() => setMethod('visa')}>
              <div className="flex items-center gap-4">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${method === 'visa' ? 'border-primary' : 'border-gray-300'}`}>{method === 'visa' && <div className="w-2.5 h-2.5 bg-primary rounded-full"></div>}</div>
