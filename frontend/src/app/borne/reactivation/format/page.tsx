...
export default function FormatReactivation() {
  ...
  const t = {
    title: lang === "en" ? "How do you want your line back?" : "Comment souhaitez-vous récupérer votre ligne ?",
    subtitle: lang === "en"
      ? "Both options reactivate the same number"
      : "Les deux options réactivent le même numéro",
    physiqueTitle: lang === "en" ? "Physical SIM" : "SIM Physique",
    physiqueDesc: lang === "en"
      ? "Get a new physical card for this number, delivered instantly."
      : "Récupérez une nouvelle carte physique pour ce numéro, délivrée instantanément.",
    esimTitle: "eSIM",
    esimDesc: lang === "en"
      ? "Reactivate directly on your compatible phone, no physical card."
      : "Réactivez directement sur votre téléphone compatible, sans carte physique.",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* SIM Physique */}
        <button
          data-ai-action="btn-reactivation-sim-physique"
          onClick={() => {
        ...
        >
          ...
          <p style={{ fontSize: "clamp(12px, 1.2vw, 14px)", color: "#9CA3AF", lineHeight: 1.6, margin: "0 0 clamp(16px, 2vw, 24px) 0" }}>
            {t.physiqueDesc}
          </p>
        ...
        {/* eSIM */}
        <button
          data-ai-action="btn-reactivation-esim"
          onClick={() => {
            sessionStorage.setItem("kiosk_flow", "esim");
            router.push("/borne/reactivation/esim/compatibilite");
          }}
          style={{
            background: "white",
            borderRadius: "clamp(16px, 2vw, 24px)",
