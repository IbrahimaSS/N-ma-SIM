...
// Poular historique (/comprendre) — on adapte ici plutôt que de toucher au
// reste du front (AgentIA.tsx, soussou-vocal.ts).
...
// (mêmes noms que ceux utilisés côté AgentIA.tsx).
...
// ─── Soussou : nouvelle API dédiée (wav2vec2, /predict) — remplace l'ancienne
// API scikit-learn (/comprendre). Fonction séparée (pas de mutualisation avec
// comprendreMalinke) pour rester lisible : contrats/labels propres à chaque langue.
const SOUSSOU_PAGE_MAP: Record<string, string> = {
  page_choix_du_service: "choix_service",
  page_type_de_piece: "type_piece",
  page_motif_reactivation: "motif_reactivation",
};

// Libellés du modèle Soussou → intentions génériques attendues par le front.
const SOUSSOU_INTENT_MAP: Record<string, Record<string, string>> = {
  choix_service: { nouvelle_sim: "nouvelle_sim", reactivation: "reactivation_sim" },
  type_piece: { carte_didentite: "carte_nationale_identite", passport: "passeport", electeur: "carte_electeur" },
  motif_reactivation: { ebarahabloquer: "blocage", abarabou: "inactivite", abaralouwai: "perte" },
};

async function comprendreSoussou(page: string, audio: Blob) {
  const vocalApiUrl = process.env.VOCAL_API_URL_MALINKE || "http://127.0.0.1:8301";
  const soussouPage = SOUSSOU_PAGE_MAP[page];

  if (!soussouPage) {
    return { intention: null, raison: "page_inconnue", message: `Page non supportée en soussou : ${page}` };
  }

  const apiFormData = new FormData();
  apiFormData.append("audio", audio, "enregistrement.wav");
  apiFormData.append("langue", "soussou");
  apiFormData.append("page", soussouPage);

  const response = await fetch(`${vocalApiUrl}/predict`, { method: "POST", body: apiFormData });
  if (!response.ok) {
    const text = await response.text();
    console.error(`API Vocale Soussou a retourné ${response.status} :`, text);
    return { intention: null, raison: "erreur_api_vocal", message: `Erreur ${response.status} Python (soussou) : ${text}` };
  }
  ...
  const intention = SOUSSOU_INTENT_MAP[soussouPage]?.[data.label] ?? null;
  return {
    intention,
    raison: intention ? "ok" : "label_non_mappe",
    scores: data.all_scores,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (lang === "sus") {
      const data = await comprendreSoussou(page, audio);
      return NextResponse.json(data, { status: 200 });
    }

    // ── Poular : ancienne API dédiée (/comprendre), inchangée ──
    const apiFormData = new FormData();
    apiFormData.append("page", page);
    apiFormData.append("audio", audio);

    const vocalApiUrl = process.env.VOCAL_API_URL_POULAR || "http://127.0.0.1:8200";
